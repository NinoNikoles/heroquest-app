<x-app-layout>
        ...
        <section>
            <div class="innerWrap">
                <div class="grid">
                    ...
                    <div class="grid 12">
                        <div class="col12">
                            <div class="character__info grid">
                                <div class="character__header col6">
                                ...
                                <div class="box col6 grid">
                                    <div class="grid character__stats col-6">
                                        <div class="character__stats--hp w-full">
                                    ...
                                    <div class="grid character__stats col-6">
                                        <div class="character__stats--int w-full">
                                                <div class="grid character__stat__base">
                    ...
                    <!-- Würfel -->
                    <div class="col12 character__dice box rounded border" x-data="{ openDice: true }">
                        ...
                        <div class="grid" x-show="openDice">
                            <div class="col4 character__dice__box box border rounded">
                                        ...
                                        <div class="grid-row-gap">
                                            @if ($character->items->where('type', $type)->count() > 0)
                                                @foreach($character->items->where('type', $type) as $item)
                                                    <div class="item flex no-wrap rounded-small border bg-01dp flex-stretch {{ $item->pivot->is_equipped ? 'bg-blue-transparent border-blue' : '' }}">
                                                        
                                                        <div class="item-image">
                                                            <figure class="square">
                                                                <img src="/resources/images/{{ \Illuminate\Support\Str::slug($item->name['en-US'], '_') }}.jpg">
                                                            </figure>
                                                        </div>

                                                        <div class="box grid flex-grow no-wrap">
                                                            <span class="grid flex-grow ">
                                                                <div class="col12 flex-center">
                                                                    <span class="special">{{ $item->name[$browserLang] }}</span>
                                                                    @if($item->type === 'weapon')
                                                                ...
                                                                @if($item->description)
                                                                <div class="col12">
                                                                    <span class="t-small">{{ $item->description[$browserLang] }}</span>
                                                                </div>
                                                                @endif
                                                            </span>

                                                            <div class="grid-gap-small flex no-wrap">
                                                                @if(!$item->pivot->is_equipped && in_array($type, ['weapon', 'armor']))
                                                                ...
                                                                @endif
                                                                @if(!$item->pivot->is_equipped && in_array($type, ['weapon', 'armor', 'potion', 'artefact']))
    ...
    <script>
        ...
        function doSearch(q) {
            ...
            fetch(`/items/search-api?query=${encodeURIComponent(q)}&lang=${userLang}`)
            .then(res => res.json())
            .then(items => {
                let html = '';

                if(items.length === 0) {
                    html = '<div class="box">Keine Schätze gefunden...</div>';
                } else {
                    items.forEach(i => {
                        // Werte vorbereiten
                        let stats = '';
